...
def broadcast(message, channel, sender=None, clientSocket=None, locks_held=False):
    timestamp = datetime.now().strftime("%H.%M")
    if channel not in messageHistory: # Create a new message history for the channel if it doesn't exist
...
def privatemessage(message, sender, receiver, clientSocket):
    ...
    with clientsLock:
        ...
        for nick in clients:
            if nick.lower() == receiver.lower(): # Check if the receiver exists
...
def handleClient(clientSocket, clientAddress):
    ...
    try:
        ...
        with acquirelocks():
            ...
            broadcast(f"{nickname} has joined the {defaultChannel}", defaultChannel, None, None, True) # Notify other channel members
            ...
            if msg.startswith("JOIN:"): # Join a channel
                    ...
                    with channelsLock:
                        ...
                        if requestChannel in messageHistory and len(messageHistory[requestChannel]) > 1:
                            ...
                            for entry in messageHistory[requestChannel]:
                                sendername = entry['sender'] if entry['sender'] else "Server" # Use "Server" if sender is None
                ...
                with clientsLock:
                    with channelsLock:
                        currentChannel = getUsersChannel(nickname, True) # Get the current channel of the user
                        if currentChannel:
                            broadcast(message, currentChannel, nickname, clientSocket, True) # Broadcast the message to the channel
                        else:
                            timestamp = datetime.now().strftime("%H.%M")
